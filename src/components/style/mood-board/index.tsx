...
const MoodBoard = ({ guideImages }: Props) => {
  ...
  return (
    <div className="flex flex-col gap-10">
            ...
            <div className="lg:hidden absolute inset-0 flex items-center justify-center">
              <div className="relative">
                {images.map((image, index) => {
                  ...
                  const rotation = (random1 - 0.5) * 20; // -10 to +10 degree for subtitle titlt
                  const xOffset = (random2 - 0.5) * 40; // -20 to +20 px horizontal offset
                  const yOffset = (random3 - 0.5) * 30; // -15 to +15 px vertivale offsets
            ...
            <div className="hidden lg:flex absolute inset-0 items-center justify-center">
              <div className=" relative w-full max-w-[700px] h-[300px] mx-auto">
                {images.map((image, index) => {
                  ...
                  const overlapAmount = 30; // reduce overlap
                  const spacing = imageWidth - overlapAmount;

                  // position from left to right with slight random rotation
                  const rotation = (random1 - 0.5) * 50; // -25 to _25 degree
                  const xOffset =
                    index * spacing - ((images.length - 1) * spacing) / 2; // center the sequence
                  const yOffset = (random3 - 0.5) * 30; // -15 to +15 px minimal verticle
                  const zIndex = index + 1; // Later images to top
      ...
      {/* todo */}
      <Button className="w-fit">Generate style with AI</Button>
